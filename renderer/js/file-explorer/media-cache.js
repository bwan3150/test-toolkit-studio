...
(function() {
  ...
  class MediaCache {
    ...
    /**
     * 获取媒体文件的本地路径（图片直接返回缓存路径，视频返回缩略图路径）
     * @param {string} remotePath - 远程文件路径
     * @param {string} deviceId - 设备ID
     * @param {string} fileType - 文件类型 ('image' | 'video')
     * @returns {Promise<string>} 本地文件路径（用于 img src）
     */
    async getMediaUrl(remotePath, deviceId, fileType) {
      try {
        // 下载原始文件到本地缓存
        const localPath = await this.pullMedia(remotePath, deviceId);

        if (fileType === 'video') {
          // 视频：生成并返回缩略图路径
          const thumbPath = await this.getVideoThumbnail(localPath, deviceId, remotePath);
          return thumbPath;
        } else {
          // 图片：直接返回缓存路径
          return localPath;
        ...
        window.rError(`获取媒体 URL 失败: ${remotePath}`, error);
        throw error;
      }
    }
