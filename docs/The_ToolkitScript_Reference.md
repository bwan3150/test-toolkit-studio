...
Param3: 移动阶段时长/ms (选填, 默认 1000ms)

**与「滑动」是两件事**：拖拽会在按下之后**先停约 600ms** 再移动 ——
拖动排序控件（Flutter 的 ReorderableListView 那类）只认这一种手势。
用「滑动」的话命令会跑成功，而顺序纹丝不动。

目前只有安卓支持（靠 `input motionevent`）；web / iOS 上会明确报不支持。
...
Param1: 等待时长 (选填, 为空则默认 1 秒)。**必须带单位**：`s`=秒、`ms`=毫秒。
        如 `5s`=5 秒、`1000ms`=1000 毫秒(即 1 秒)。也可填已落库元素名以等待其出现。
