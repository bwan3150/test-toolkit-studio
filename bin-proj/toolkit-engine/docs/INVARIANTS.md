...
- 「tke 不再透传原生工具，设备操作一律走 tke 指令」+ 四行指路（打错一个命令时）
  ——他要的是"我打错了"，不是 tke 的设计史（2026-08-30 用户指出）
- 「⏹ 收到中断（Ctrl+C）——将在当前步骤结束后安全停止…（再按一次立即退出）」
  ——现在是「安全中断中…  再按一次立即中断」：在干什么 + 想更快怎么办，别的都是废话

判据一句话：**这句话会不会改变他下一步做什么。** 不会就是占地方。

**这条也管 `--help`**（用户 2026-08-30 拿 zellij 对照）：

- 形照 clap 的惯例：`USAGE` 在最前，命令与参数各成一段，一条一行、动词开头
- 描述里不写背景（「唯一会联网下载的命令」「老写法照常能用」）
- **不写内部编号**：`（ADR-0025）` 对敲命令的人没有意义
- 子命令自己的 `--help` 里不重复分类前缀（`[工具]`）——他已经敲了那条命令
- 例子放 doc comment 的**第二段**（clap 会摆到 Usage 之后），
  塞在标题那一行会跟 `Usage:` 抢地方
- 颜色只用三种（段标题 / 命令名 / 次要说明）；多了就不是区分而是装饰
