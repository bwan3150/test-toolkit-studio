...
## D7（2026-08-26 追加）：节点主动注册 + 心跳，不做平台轮询

**用户拍板**："轮询不稳定而且容易炸服务器"。

轮询是平台去敲 N 个节点：节点挂了要等超时、节点多了是 N 倍请求、平台还得维护一份
"有哪些节点"的清单。心跳反过来——**节点自己报，死了就是不报了**，平台只需要一张表
和一个超时判据（超过 3 个心跳周期没消息判 offline）。

### 协议：一个端点，幂等 upsert

`POST <平台>/api/v1/node/heartbeat`，**第一次心跳就是注册**。节点不需要知道自己的
node_id，也就不需要"先注册拿 id 再心跳"那一步——少一个会失败的环节，少一份要持久化的状态。

- **每次带全量设备清单**，平台整份替换。事件式（插一台推一条）漏一条就永久错位，
  全量替换会自愈：下一次心跳就对上了。
- **节点自报可达地址**（`--advertise`）：监听地址多半是 `0.0.0.0`/`127.0.0.1`，平台猜不出来。
- **周期由平台回**（`interval_s`）：想让所有节点降频不用挨个改配置。
- 响应带 `tasks: []` 恒为空——给"NAT 后的节点自己拉任务"留的口子。**现在就把字段放进去**，
  将来启用不用改协议版本。

### 这条强化了 D1 的边界

节点现在持有两样凭据：`--token`（平台调它）、`--platform-token`（它报平台）。
**业务凭据仍然一样都没有**——不认识 App、不认识用户、不持有 AI key、不持有上传凭据。
配套地：产物是**平台去拉**（`GET /sessions/{sid}/artifacts/**`），不是节点推。

**加任何"给节点配个 xx"的需求之前，先想能不能改成随任务下发。**

### 代价

- 多一份配置（平台地址 + 报到凭据）要在节点上放，且要先在平台建好节点行拿 token——
  不接受陌生节点自己冒出来（否则任何知道地址的人都能往池子里塞设备）。
- **不解决 NAT**：下发任务仍是平台 → 节点。真要支持用户自带的、在 NAT 后的真机墙，
  出路是让心跳响应捎上待办任务，节点自己拉（口子已留，现在不实现）。
- 连不上平台时节点照常独立可用——报不上去只是平台看不见它。这是有意的：
  **平台是可选的增强，不是节点的运行前提。**
