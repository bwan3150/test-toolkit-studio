...
```bash
tke update
systemctl --user restart tke-node
```

**两条都要。** `tke update` 换的是 `~/.tke/bin/tke` 这个文件，而正在跑的进程
握着旧的那一份（Linux 上换掉可执行文件不影响已经在跑的进程）——
不 restart 的话，节点还是旧版本在跑，而 `tke --version` 显示的是新的。

平台的「云设备」页显示的 `tke` 版本来自节点自己上报，所以那里也会跟着一起变 ——
**没变就说明没重启**。
...
- **起了新模拟器不用重启**：设备池每 30 秒重扫一次，半分钟内平台上就会多出来
  （0.7.5 之前是启动扫一次就定死，那时候必须重启）
