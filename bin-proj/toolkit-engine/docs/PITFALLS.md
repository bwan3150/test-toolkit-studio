...
## P-60 (2026-08-27) `--cache` 的文档是真的，代码不是 —— 采集产物落在共享目录里

**现象**：平台想在云设备页「点一下截一张」。`tke serve` 给每个会话分了独立
cache 目录，`refresh` 也报 `success:true` 并回了截图路径，但平台按会话去取产物，
`/v1/sessions/{sid}/artifacts` 列出来是**空的**。

**根因**：`Workarea::for_device` 里写死了 `$TMPDIR/tke/workarea/<设备>`，
**根本不看 `--cache`**。而 `Params::cache_root()` 的文档注释白纸黑字写着
「截图/页面结构/会话日志/临时元素库等都落这里」。本地验证：
`tke -d web --cache <空目录> refresh` 跑完，那个目录里一个文件都没有。

两个后果，第二个更要命：

1. 平台按会话取不到产物（云设备的按需截屏就是卡在这儿）
2. **上一个租户的屏幕留给下一个租户**。设备级共享目录不随租约走、租约结束也不清，
   下一个人在同一台设备上 `fetch --cached` 读到的是别人的页面 ——
   正是 INV-17 说的「设备脏状态」，而且是网络可达的那种

**规则**：**目录参数的文档说了算，就得让代码说到做到**。
`--cache` / `--current-dir` / `--log` 这几个是给调用方划边界用的，
有一个不认账，整套沙箱假设就不成立 —— 而且不成立的地方**不会报错**，
只会在别人按契约取东西时表现成「什么都没有」。

新增/改动任何写文件的地方，先问一句：它落在哪个根下、那个根是不是调用方指定的。
...
## P-63 (2026-08-28) 同一台设备上并发两条命令：不报错，只是回一张空表

云设备工作台进去之后元素面板一直空着，手动点一次"刷新"却能出来。
查到最后是调用方**同时**发了两条：

```js
await Promise.all([shoot(), loadElements()])   // 截图 + 取元素
```
...
```
并发 → 元素响应 353 字节（空的）
串行 → 元素响应 3824 字节（真的元素表）
```

**最贵的地方是它不报错**：`fetch` 照样 `exit_code: 0`，只是里面什么也没有。
调用方看到的只有"元素面板莫名其妙空着"，谁也想不到是自己并发发的两条请求。
写那行 `Promise.all` 时的理由是"两条独立命令，串行等于把等待时间加倍" ——
那句话是错的：截图要 refresh 设备、取元素要 dump 界面，**它们抢的是同一台机器**。

两头都改了：

- 调用方改串行；
- **tke serve 加了一把会话闸**（`LeaseTable::gate`）：同一个租约一次只跑一条命令。
  闸门开在这一层，而不是指望每个调用方自己记得串行 —— 这种约定没有任何地方能强制，
  而违反它的表现又不像出错。（只管命令层 exec/screen；L2 的 harness 持有租约的
  整个生命周期，把它圈进来的话任务跑着时连截张图都会被堵死。）

**教训**：共享资源上的竞争如果表现为"结果变空"而不是"报错"，它就会一直被当成
别的问题去查。能加闸的地方加闸，别靠调用方自觉。

---

## P-64 (2026-08-28) 死代码里的"正确实现"，和真正在跑的那两份内联

元素面板里连着三行「—」：安卓最常见的行结构是「可点的 LinearLayout +
里面一个不可点的 TextView」，文字长在子节点上，`fetch --interactive` 只留可交互的，
于是那些行一个字都没有 —— 只能退回按坐标点（最脆的定位方式）。

给 `Fetcher::filter_interactive_elements` 加了"继承子孙节点文字"，构建、真机跑，
**没有任何变化**。原因：那个函数**一个调用方都没有**，真正在跑的是
`fetch.rs` 里内联的两份 `.filter(|e| e.clickable)`。我改的是死代码。

顺带还暴露了另一件事：那个没人用的函数写的是
`clickable || focusable || checkable || scrollable`，跟真正在跑的 `clickable`
根本不是一个判据 —— 两份实现早就漂开了，只是没人发现，因为其中一份从来没被执行过。

**教训**：改一个"看起来是这件事的实现"的函数之前，先 grep 一下谁在调它。
一个 `pub fn` 没有调用方，多半不是"给外部用的"，而是**已经被绕过去了**。
（现在合成一份自由函数 `interactive_view`，两处都调它。）

---

## P-65 (2026-08-28) 节点被 kill -9，它起的 harness 还握着设备继续跑

节点重启后设备行为诡异，查下来是上一代的 `tke harness` 子进程还活着 ——
一次清掉过 3 个。它们仍在 adb 点点点、仍在写会话目录，而新节点对它们一无所知。

`kill -9` 时父进程没有任何机会做收尾，所以**在父进程里写清理代码是没用的**。
两条防线，缺一不可：

1. **`PR_SET_PDEATHSIG`**（Linux）—— 父进程一死内核就给子进程发 SIGTERM，
   `kill -9` 也拦不住这条。实测：节点被 kill -9 后 harness 5 秒内自己退出。
   macOS/Windows **没有等价物**。
2. **启动时收尸**（三平台通用）—— 子进程 pid 落在会话目录，节点启动先扫一遍
   把上一代还活着的清掉。实测日志：`清掉上一代遗留的任务进程 pid=3387392 session=…`。

只有 1 会漏掉 mac；只有 2 会留下一个"节点已经死了、孤儿还在操作设备"的窗口。

**收尸的唯一危险是 pid 复用**：pid 号还在，但那已经是别人的进程了，照着杀就是误杀。
所以判据不能是"pid 还在"，而是**命令行里同时含 `tke` 和这个 session id** ——
session id 出现在子进程的 `--log`/`--cache` 路径里，是这一次任务独有的指纹。

**顺带**：`pgrep -f "tke serve …"` / `pkill -f` 会匹配到**发起命令的那个 shell 自己**
（它的命令行里就有这串），`kill` 下去把自己杀了，退出码 144。
HANDOFF 里早就记着这条，这一场又踩了两次 —— 验证脚本里尤其容易中招，
因为脚本正是围着这个进程名转的。

---

## P-66 (2026-08-28) 释放会话没停任务：设备"还回去了"，可它还在被人操作

用真账号跑通一次对话式探索、点「归还」，之后**所有新任务都起不来** ——
平台侧只留下一个空的 `failed`（outcome 空、detail `{}`、session_id 空），
没有任何一句话说明为什么。

查下来：上一场的 `tke harness` 子进程还活着，还占着那台 web 设备。
`session_delete` 做的是「摘租约 + 复位设备」，**唯独没停这个会话上正在跑的任务**。

于是三方各自都"没错"：
- 节点：租约摘了，设备回池子了
- 平台：设备显示空闲，可以派活
- 而那个 harness：还在这台设备上点来点去

**释放一份资源，必须同时停掉正在用它的东西**，否则"已释放"只是账面上的。
现在 `session_delete` 先 `kill_session_task`（复用收尸那套判据：命令行含 tke +
session id，防 pid 复用误杀）再复位 —— 顺序也不能反，任务还在跑的话，
复位命令会和它在同一台设备上打架。

**这个坑最贵的地方是它不报错**：错误信息是一个空的 `failed`，
而真正的原因（设备被一个理应死掉的进程占着）在任何日志里都看不见。

---

## P-67 (2026-08-28) 交互式会话的产物，平台从来没拉过

云设备页起的对话式 agent，探索产出的 `.tks` + `.tklib` 和报告
**全留在节点的会话工作区里**，平台一个字节也拿不到。

排期任务（run dispatch）那条路一直是好的 —— scheduler 跑完会 `pullArtifacts`。
而交互式这条路走的是 `device_console_handler.release`（`Finish` + `CloseSession`），
从头到尾没有拉产物这一步。

后果：**探索完的东西没有任何办法进平台**。实测时只能手工去
`~/.tke/serve/sessions/<sid>/workspace/` 把两件套捞出来，再调 API 建成脚本 ——
而正常用户没有这条路。

修法是在 `release` 里补一次拉取，但**顺序是关键**：要在 `CloseSession` **之前**，
会话一关工作区随时会被清掉。产物拉取抽成了 `scheduler.Puller`，两条路共用一份 ——
复制第二份实现的话，上限、命名、登记规则会慢慢漂开，而漂开的表现是
"某一条路上的产物少了几个"，没人看得出来。

**教训**：一条链路做完之后，问一句"另一条入口有没有同样的收尾"。
这两个坑（P-66/P-67）是同一个形状：**排期那条路想周全了，交互式那条路漏了**。
