...
| **安卓模拟器(AVD)** | 🟡 已实现,**待真机验**(本机没装 SDK) | **ADR-0018:选装**——iOS 模拟器 macOS 自带,而这套要 1GB 上下(emulator 包 351~490MB + aosp_atd 镜像 450~860MB),安卓真机又很好开。**不分发、不进依赖检查**,doctor 只写「未安装（选装）」。`-d avd:<名字>` + `启动环境` 起它(等 `sys.boot_completed`),起来后就是普通 adb 设备。**Google 不发布 linux-arm64 emulator**,那一档只能用 redroid |
