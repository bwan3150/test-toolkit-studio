...
| **doctor 报告排版** | ✅ 本机实测(Linux) | 用户说"信息太多而且顺序混乱"→ **三段分组**:①工具本身(平台/依赖/Engine/Skill 版本)②能测什么(四端+显示器,真机在前)③落点(Engine/Skill/日志);标签列按显示宽度对齐、值统一 `状态 (补充)`;**正文一条命令都不留**,该敲的全收进末尾「下一步」块;**语气色**(绿就绪/黄有更新/红缺依赖/灰不可用),队形三条:标签一律 dim、值上色、补充永远 dim。设备探测改走 `tools::discover`(与 `device list` 同一套)。排版拆到 `cli/doctor.rs` |
