...
| **启动环境** | `启动环境` | ❌ | — | ✅ | ✅ | 起浏览器 / 开模拟器。**与 `启动` 是两件事**：那个在环境里打开 App/网址，这个把环境本身弄起来。真机不需要（插上即用）；**安卓模拟器暂不支持**（要 Android SDK 的 emulator 二进制 + AVD 名） |
| **关闭环境** | `关闭环境` | ❌ | — | ✅ | ✅ | 关浏览器进程 / 模拟器关机。**与 `关闭` 分开**：那个关的是环境里面的 App/标签页 |
