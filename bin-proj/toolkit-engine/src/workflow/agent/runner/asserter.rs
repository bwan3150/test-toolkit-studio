        ...
        Some(idx) if idx < p.elements.len() => {
            ...
            match execution::apply(ctx.device, ctx.element_path, &act, &p.elements, known_names, &p.shot_path, tx, round).await {
                Ok((line, detail, trace, saved)) => {
                    ...
                    s.steps.push(StepResult { index: step_index, command: line.clone(), success: true, error: None, duration_ms: 0, line: None, screenshot, xml, healed: None, note: None, dialog: None, errors: vec![] });
