...
pub(super) async fn optimize(
    ...
    let diag = super::diagnose::diagnose(tx, ctx, params, case, lines, marker, start_marker, None, "reflect_diagnose", 0, false).await;
    if !diag.reached {
        return None;
    }
    ...
    let verify = super::diagnose::diagnose(tx, ctx, params, case, &new, marker, start_marker, None, "reflect_verify", 0, false).await;
    if !verify.reached {
