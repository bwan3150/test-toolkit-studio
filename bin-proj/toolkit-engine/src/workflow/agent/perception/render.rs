...
/// 渲染元素列表：`[序号] 控件描述 @(中心坐标)`
/// known[i] 为该元素在元素库中已存在的 name（命中结构/ocr），标注让 AI 复用、不重复造。
pub fn render_element_list(elements: &[UIElement], known: &[Option<String>]) -> String {
    ...
    for (i, el) in elements.iter().enumerate().take(MAX_ELEMENTS) {
        ...
        let tag = match known.get(i).and_then(|k| k.as_ref()) {
            Some(name) => format!(" ←已知元素「{}」，name 请复用它", name),
            None => String::new(),
        };
