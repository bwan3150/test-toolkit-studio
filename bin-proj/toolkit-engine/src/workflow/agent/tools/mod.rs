...
pub fn build_tools(prompts: &PromptSet, platform: crate::Platform, with_source: bool) -> Vec<LlmTool> {
    const NO_COMMENT: [&str; 4] = ["finish", "ask_user", "request_screenshot", "changed_surfaces"];
    schema::tool_schemas(platform, with_source)
        .into_iter()
        .map(|mut t| {
