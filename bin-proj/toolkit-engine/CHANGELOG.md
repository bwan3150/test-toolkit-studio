...
- **`--help`**：加 `USAGE` 段，命令按原子/工作流/安全/环境/工具/服务分组，
  一条一行、动词开头。删掉所有背景交代（「唯一会联网下载的命令」「老写法照常能用」）。
  颜色收敛到三种：段标题黄 / 命令名青 / 次要说明灰 —— 多了就不是区分而是装饰。
  补上了漏列的 `http` / `recon` / `sandbox` / `serve` / `remote`。
- **打错命令**：从「未知命令 + tke 不再透传原生工具 + 四行指路」
  改成 clap 那一套：`error: 不认识的命令 \`h6\`` + `USAGE:` + 一句 `tke --help`。
  直通为什么删了是 ADR-0015 的事，不是打错一个命令的人要读的。
- **中断**：`安全中断中…  再按一次立即中断`。在干什么 + 想更快怎么办，
  不解释为什么要等。
- 顺带清了子命令 `--help` 里的两类冗余：分类前缀 `[工具]`（他已经敲了那条命令）、
  内部编号 `（ADR-0025）`；描述里的例子移到 doc comment 第二段，
  否则会跟 clap 自己生成的 `Usage:` 抢那一行。

INV-18 补了「这条也管 `--help`」一节 + 三条新反例。
...
P1 只做变更面聚焦，**只靠文件路径映射，不解析源码**：
`*Activity` / `res/layout/*` / `*ViewController` / `pages|views|screens|routes/*.vue|tsx`。
宁可漏不可错 —— 报一个不存在的界面，AI 会去找一个不存在的东西，比没有线索更慢。

**INV-19 靠结构兑现**：`Surface` 只有名字/类型/文件/行数四个字段，没有内容字段；
给 AI 的唯一出口 `render_for_ai` 带一条"渲染里不能有代码"的测试；
无设备回归断言源码内容一个字节都不进会话。

`tke serve --sandbox` 才启用（默认不启用：随手接上来的机器不该自动开始存别人的源码）。
`--source-tree/--source-base/--tree` 进 BANNED_FLAGS —— 落点由服务端按租约算，
让调用方指定等于让它说"去读这台机器上的哪个目录"。
自查入口：`tke sandbox surfaces --tree . --base main`。

**判据还没量**：探索轮数 / token 降幅要同一个用例在有无源码下各跑一次。P2 之前先量它。

### 2026-08-28 · 版本串带上构建号
`0.7.4-beta` 整个 beta 期不变，于是**"这台节点装没装上某个修复"谁也说不清** ——
实测吃过两次亏：用户更新完节点报"还是那个毛病"，而两头都无法判断他拿到的是哪一版，
只能靠猜（一次猜错，白查了半天平台侧）。

改成 `0.7.4-beta (b5d3b98f)`。**并进同一个串**而不是加字段：
平台那边零改动就看得见（不用加列、不用迁移），设备管理页直接显示。
拿不到 git（下载 tarball 构建）就只显示版本号，不让构建失败。


### 2026-08-28 · 对话外壳对 `--json` 也开 + 模型兼容改成探测回退

用户拍板的两件事。

#### 一、`--json` 那条路也进 REPL

对话外壳**本来就有**（`orchestrator::serve` 里那段 REPL 等待），
只是判据写的是 `is_interactive()` —— 而 `JsonFrontend` 只有 `supports_prompts()=true`。
于是 Studio 桌面 app 与 serve/API 那条路**跑完一轮就退出**：
云设备页上表现成"答完一件事，对话就没了、输入框变灰"。

**判据该问的是"有没有人能回答我"**，不是"是不是全屏 TUI"。改成 `supports_prompts()`。
Plain（管道 / CI）仍然一轮结束 —— 那边真的没人可问，等 stdin 会永久挂起。

`finish` 也跟着改：**它是"这件事做完了"，不是"这场对话结束了"**。
有人能回答就收尾后继续等，不退场 —— 一做完就退出的话，人想接着说下一件
只能重起一场，而重起等于失忆（设备状态还在，AI 的上下文没了）。

#### 二、模型兼容：**名单退场，改探测 + 回退**

那张"哪些模型不支持 reasoning"的名单栽过两次：
按 `gpt-4*` 挡 → `gpt-5.4-mini` 按名字放行然后 400；
改成按 provider 挡整个 OpenAI → 一刀切，把本来能用思考的路也关了。

**名单永远追不上上游。** 现在照常发，上游若拒绝（回话里指名 `reasoning_effort`）
就摘掉这个参数重试一次并记下，本场不再发。

代价：不兼容的组合每场多一次被拒的请求（400 不产生 token）。
换来的是**新模型出来不用改代码**。

判据只认"提到了这个参数名"，不解析措辞 —— 措辞会变，参数名不会。
配套两个测试：认得出两种真实回话、别的错误不误触发。

真机验过（`gpt-5.4-mini`，此前起步就挂）：跑完一件事、出了脚本与 summary，
然后**停在空 `awaiting_input` 上等下一句** —— 会话没有结束。
...
返回的是"能跑什么"，不是"怎么跑"（参数还是看 `--help`）。

不需要租约：它跟某台设备无关，是这个节点的能力清单。
...
### 2026-08-26 · fix 分发源漏传 manifest —— 安装器一直只装了一个 skill（P-55）
用户 mac 上 `tke doctor` 只有一个 skill 落点：`tke-security-test` 明明打了包也传上去了
（分发源上 9848 字节、取得回来），却谁也装不到。
- **根因**：`install.sh` 读 `<分发源>/skills` 这个 **manifest 文件**决定装哪些，而 CI 的上传
  那行只有目录 `dist/skill`、**没有文件 `dist/skills`** → 分发源 404 → 走兜底只装 ui-test。
  名字差一个 s、又平级同名，写清单时眼睛会自动把它们归成同一样东西
- **兜底不报错**，于是三方都显得正常：CI 绿、安装成功、doctor 说"已是最新"
- **复验也够不着它**：`find bin skill -type f` 遍历的是两个**目录**，`skills` 在它们之外——
  "复验分发源"那一步给了虚假的安心
- **三处一起修**：①CI 上传清单加 `dist/skills` ②复验**逐个名字比对 manifest 内容**
  （平台对不存在的路径回落 200+HTML，P-19，"取回来了"什么都不证明）③兜底往 stderr 说一句
- 手册里"分开传"的例子同样漏了它，照着敲会踩同一个坑，一并修
- **本地起分发源两个场景都验过**：manifest 在 → 四个 skill 全装上（含两个 remote）；
  manifest 缺 → 明说"只装 tke-ui-test"（以前静默）
- **共性记进 P-55**：凡是写 `|| 默认值` 的地方，问一句「默认值生效时有没有任何人看得出来」

### 2026-08-26 · 节点主动向平台报到（心跳）—— ADR-0023 D7
用户拍板：**不要平台轮询**（"轮询不稳定而且容易炸服务器"）。轮询是平台去敲 N 个节点，
节点挂了还得等超时、节点多了是 N 倍请求；心跳反过来——节点自己报，死了就是不报了。
- **`src/serve/heartbeat.rs`**：`tke serve --platform <平台> --platform-token <t>
  [--node-name] [--advertise]`。一个端点、幂等 upsert，**第一次心跳就是注册**
  （节点不用知道自己的 node_id，少一个会失败的环节、少一份要持久化的状态）
- **每次带全量设备清单**：事件式（插一台推一条）漏一条就永久错位，全量替换会自愈
- **自报可达地址**（`--advertise`）：监听地址多半是 `0.0.0.0`，平台照着它够不着
- **周期由平台回**（`interval_s`），节点照做——想让所有节点降频不用挨个改配置
- **连不上平台不影响节点干活**：只记日志 + 退避（封顶 2 分钟），且**只在第一次和每 20 次
  失败时喊**（平台维护一小时的话，每 15 秒一条 WARN 会把日志淹掉而信息量只有第一条）
- `PoolDevice` 补 `model`/`os`：分开留着而不是拼进 label——平台的设备表要按机型筛、
  按系统排，拼成一个串就切不开了
- 地址与凭据**只给一个直接报错**，不静默不报到（那会让人对着平台上空空的节点列表查半天）
- **本机端到端验过**：真 tke 报到 → 平台 online + 设备进池 → 杀掉节点 → 56 秒后自动判 offline
- 平台侧对应实现在 `TOOLKIT/bug`（迁移 116 + `api/node`），commit `e0c522a`

### 2026-08-26 · AI 走平台网关（凭据不下发）+ `meta` 归账透传
平台的 App 下本来就有用户自己填的 API Key，账要记在那个 App 名下
（App 总用量 > 个人 > session；设备租赁与 AI 计费分开记录、共用一套组件）。
最初做成"平台把 key 随任务交给节点"，用户提出更好的形状：**平台出 AI 网关**，
节点只调网关地址——**用户的 key 一步都不离开平台**，计量也变成网关侧权威的。
- **fix/feat `[ai].base_url` 放开给所有 provider**，且**保留各家原生适配器**（只换端点不换协议）。
  原本硬限制在 doubao/qwen 上并强制换成 OpenAI 适配器——那等于把网关这条路堵死了。
  **必须保原生**：走 OpenAI 兼容会丢 anthropic 的思考块（历史坑：genai 丢思考块 →
  anthropic 必须 4.6+adaptive），而思考块正是 harness 质量的来源之一
- **feat `AiConfig::apply_env()`**：`TKE_AI_PROVIDER/MODEL/KEY/BASE_URL/REASONING` 覆盖配置文件。
  远程任务层每次任务换一把凭据走的就是这条——**`--ai-key` 会出现在 `ps aux` 里**，
  写临时配置文件会落到磁盘上，环境变量是三者里唯一不留痕的。空串等于没设
  （否则 `TKE_AI_KEY=` 会把节点自己的 key 抹成空的，报错还是"没有 key"，完全摸不着头脑）
- **feat 任务层收 `ai` 与 `meta`**：`ai` 经环境变量交给子进程（起进程前先清干净上一次的）；
  `meta` 是**不解释、原样带回**的归账标签，出现在任务视图/列表/webhook 与**会话（设备租赁）**里——
  tke 不认识"用户"（ADR-0022 D1），归账靠调用方自己带的这张纸条，
  **设备租赁与 AI 计费共用同一条透传路**
- **凭据脱敏**：stderr 尾巴会原样回给调用方（任务挂了它是唯一线索），所以令牌在**进缓冲之前**
  就抹掉；长度 <8 的不抹（那多半是占位符，全局替换会把正常文本切碎）
- **ADR-0023 加 D6** 记录这次形状变化；平台侧 `bug/docs/11_device_cloud.md` 补 §4.5/§4.6
- 测试：env 覆盖 1 + 适配器 2 + 脱敏/归账单测 2 + 黑盒 2（**凭据不许从任何出口漏回来**、
  `meta` 原样进 webhook 与会话）。全量 **279 绿**
